package com.example.local_syogi.presenter
...
class GameLogicRatePresenter(private val view: GameViewRateContact.View, private val boardRepository:BoardRepository): GameViewRateContact.Presenter,SocketRepository.presenter {
    ...
    override fun onTouchEvent(x:Int, y:Int) {
        if(turn == BLACK) {
            //持ち駒をタップした場合は持ち駒を置ける場所を返す
            if (y == 0 || y == 10) {
                getHintHoldPiece(x, y)
            }
            //盤上
    ...
    override fun drawView(){
        ...
        boardRepository.getAllHoldPiece(BLACK).forEach { (key, value) ->
            if(value != 0) view.drawHoldPieceBlack(key.nameJP, value, count)
            count++
        }
        count = 0
        boardRepository.getAllHoldPiece(WHITE).forEach { (key, value) ->
            if(value != 0) view.drawHoldPieceWhite(key.nameJP, value, count)
            count++
        }
    }
    ...
    //持ち駒を使う場合
    private fun getHintHoldPiece(x:Int, y:Int){
